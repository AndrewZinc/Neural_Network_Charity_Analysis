...
For the initial data examiniation, there were two columns that were immediately removed because they added no relevant data for the Deep Learning model. Categorical data columns were examined and bins were created for rare types of values.  The categorical values were then encoded to enable processing through the model.
...
The data collected during the model operation can be examined in detail within TensorBoard.  Below we can see detail of the hidden layer operations during the first optimization attempt.  Notice that the ouptput layer displays a clear Sigmoid function, as expected:
...
The Deep Learning model did not achieve the expected accuracy level maybe because of inadequate input data.  There was no data that provided information about the financial status of the recipient organizations, their abilities to manage the funds, or their budget vs operational funds.

Perhaps this information is collected by the Charity organization's application process, but if not, it is recommended for future collection.